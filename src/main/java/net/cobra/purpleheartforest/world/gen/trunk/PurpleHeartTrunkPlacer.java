...
public class PurpleHeartTrunkPlacer extends TrunkPlacer {
    ...
    public static final MapCodec<PurpleHeartTrunkPlacer> CODEC = RecordCodecBuilder.mapCodec(
            instance -> fillTrunkPlacerFields(instance)
                    ...
                    .apply(instance, PurpleHeartTrunkPlacer::new)
    );
    ...
    public PurpleHeartTrunkPlacer(int baseHeight,
    ...
    protected TrunkPlacerType<?> getType() {
        return ModTrunkPlacerType.PURPLE_HEART_TRUNK_PLACER_TYPE;
    }
    ...
    public List<FoliagePlacer.TreeNode> generate(
        ...
        list.add(this.generateBranch(world, replacer, random, height, startPos, config, function, direction, i, i < l - 1, mutable, 2));
        if (bl2) {
            list.add(this.generateBranch(world, replacer, random, height, startPos, config, function, direction.getOpposite(), j, j < l - 1, mutable, 2));
        }
    ...
    private FoliagePlacer.TreeNode generateBranch(
        ...
        // Ensure branches are vertical by setting the axis to Y for log blocks
        Function<BlockState, BlockState> verticalAxisFunction = state -> state.withIfExists(PillarBlock.AXIS, Direction.Axis.Y);

        for (int l = 0; l < k; l++) {
            this.getAndSetState(world, replacer, random, mutablePos.move(direction), config, verticalAxisFunction);
        }
        ...
        while (true) {
            ...
            if (m == 0) {
                // Generate new branches from this branch point if depth allows
                if (depth > 0) {
                    for (int n = 0; n < 2 + random.nextInt(2); n++) { // Generate 2-3 new branches
                        Direction newDirection = Direction.Type.HORIZONTAL.random(random);
                        generateBranch(
                                world,
                                replacer,
                                random,
                                height / 2, // Reduced height for smaller branches
                                mutablePos.toImmutable(),
                                config,
                                verticalAxisFunction,
                                newDirection,
                                random.nextInt(3) + 1, // Random offset for new branches
                                false,
            ...
            this.getAndSetState(world, replacer, random, mutablePos, config, bl2 ? Function.identity() : verticalAxisFunction);
        }
    }
